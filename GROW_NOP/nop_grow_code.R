...
text<-read.csv("GROW_NOP/nop_data/text-reducer-pike-texts.csv") %>%
...
#write.csv(text_bad,  "/Users/katelynking/Desktop/grow_pike_text_review.csv", row.names = FALSE )
...
manual_length1<-read.csv("GROW_NOP/nop_data/grow_pike_text_review.csv", na.strings = c("", "NA")) %>%
  drop_na(data) %>% 
...
#check this subject it is weird 59687840 add in comments
...
dropdown<-read.csv("GROW_NOP/nop_data/dropdown-reducer-pike-dropdowns.csv", na.strings = c("", "NA")) %>%
...
age_good<-filter(dropdown, review >= 2 | review2 >= 2 | (review == 2 & is.na(review2)) ) %>%
  ...
  mutate(age= ifelse(subject_id == "59963995" & column_name == "Second_row age group ", 1, age), # two NAs
         age= ifelse(subject_id == "59963995" & column_name == "Third_row age group ", 3, age)
  ) %>% 
...
#write.csv(age_bad, "/Users/katelynking/Desktop/grow_pike_age_review.csv", row.names = FALSE)
age_manual<-read.csv( "GROW_NOP/nop_data/grow_pike_age_review.csv") %>% #after manual review 
  drop_na(age) %>% 
  drop_na(subject_id)%>% 
...
all_grow<-left_join(age_data, lengths_clean) %>% 
  ...
  rename(age_group1 = " age group ") %>% 
  mutate(age_group = ifelse(is.na(age_group), age_group1,  age_group ))%>% 
  select(-c(row, age_group1)) %>% 
  mutate(length_mean= ifelse(subject_id == "58746485" & age_group == 2, "22.4", length_mean), # typo 
         length_mean= ifelse(subject_id == "61362074"& age_group == 3, "20.2", length_mean), # typo 
         length_mean= ifelse(subject_id == "58754429" & age_group == 8, "30.8", length_mean)) %>% # typo 
  filter(ifelse(subject_id == "59962968", fish_count == 2, TRUE) ) %>%  #removing the second set of data on the cards, appears to be for a different species based on lengths
...
#final data set 
write.csv(GROW, "GROW_NOP/nop_grow_qaqc.csv", row.names = FALSE)

#### add in December data #### 
#*text (length info) cleaning ####
text<-read.csv("GROW_NOP/nop_data/text_reducer_pike_dec.csv") %>%
  select(subject_id, task, 'data.aligned_text', 'data.number_views', 'data.consensus_score','data.consensus_text', "URL.4", "URL.8") %>%
  drop_na('data.number_views') %>% 
  naniar::replace_with_na(replace = list(data.consensus_text = c('-', '--', "---", '...', '. . .', "----", "-----"))) %>% #function that replaces all of a value with NA 
  drop_na('data.consensus_text') %>%
...
grow_age_values<-read.csv("GROW_BLG/BLG_data/GROW_age_values.csv", header=TRUE)
#separate into four columns, one for each of the reviews if there are different answers
dropdown<-read.csv("GROW_NOP/nop_data/dropdown_reducer_pike_dec.csv", na.strings = c("", "NA")) %>%
  left_join(grow_task_values) %>% 
  select(subject_id, column_name, 'data.value') %>%
  drop_na('data.value') %>% 
  mutate(data_value = gsub("'0 / 0'", "0", data.value), #fix hex codes 
...
#looks like the age2 are the correct answers, so choose these, and then all age1 except 82941746 are correct 
age_manual<-age_bad %>% 
  mutate(age= ifelse(is.na(review2), age1, age2)) %>% 
  select(subject_id, column_name, age) %>% 
  mutate(age = ifelse(subject_id == "82941746" & column_name == "9th_row age group ", 12, age))
...
grow_nop<-read.csv("GROW_NOP/nop_grow_qaqc.csv")

grow_join<-rbind(grow_nop, GROW)

write.csv(grow_join, "GROW_NOP/nop_grow_all_qaqc.csv", row.names = FALSE)
