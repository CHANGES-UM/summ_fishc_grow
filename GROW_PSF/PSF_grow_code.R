### Pumpkinseed sunfish GROW CARDS ###
...
text<-read.csv("GROW_PSF/PSF_data/text-reducer-Pseed-texts.csv") %>%
...
#write.csv(text_bad,  "/Users/katelynking/Desktop/grow_Pseed_text_review.csv", row.names = FALSE )
...
manual_length<-read.csv("GROW_PSF/PSF_data/grow_Pseed_text_review.csv", na.strings = c("", "NA")) %>%
...
dropdown<-read.csv("GROW_PSF/PSF_data/dropdown-reducer-Pseed-dropdowns.csv", na.strings = c("", "NA")) %>%
...
#write.csv(age_bad, "/Users/katelynking/Desktop/grow_Pseed_age_review.csv", row.names = FALSE)
age_manual<-read.csv( "GROW_PSF/PSF_data/grow_Pseed_age_review.csv") %>% #after manual review 
...
pivot_longer(-subject_id,
  ...
  drop_na("age_group") %>% # one NA row shouldn't be there 
  mutate(length_mean = ifelse(subject_id == '59964198' & age_group == 7, 6.794, length_mean), #correction 
         length_mean = ifelse(subject_id == '59964445' & length_mean == 8.5, NA, length_mean), #likely a typo because out of the range, change to NA
         length_min = ifelse(subject_id == '61362084' & age_group == 3, 104, length_min), #this whole card is in mm
         length_mean = ifelse(subject_id == '61362084' & age_group == 3, 104, length_mean), 
         length_min = ifelse(subject_id == '61362084' & age_group == 4, 145, length_min), 
         length_max = ifelse(subject_id == '61362084' & age_group == 4, 168, length_max), 
         length_mean = ifelse(subject_id == '61362084' & age_group == 4, 157, length_mean),  
         length_min = ifelse(subject_id == '61362084' & age_group == 5, 150, length_min), 
         length_max = ifelse(subject_id == '61362084' & age_group == 5, 208, length_max), 
         length_mean = ifelse(subject_id == '61362084' & age_group == 5, 160, length_mean),  
         length_min = ifelse(subject_id == '61362084' & age_group == 6, 173, length_min), 
         length_max = ifelse(subject_id == '61362084' & age_group == 6, 201, length_max), 
         length_mean = ifelse(subject_id == '61362084' & age_group == 6, 188, length_mean), 
         length_mean = ifelse(subject_id == '61362084' & age_group == 7, 227, length_mean), 
         length_mean = ifelse(subject_id == '61362084' & age_group == 8, 236, length_mean)) 
...
#final data set 
write.csv(GROW, "GROW_PSF/psf_grow_qaqc.csv", row.names = FALSE)


### ADD DEC DATA ####
#* text (length info) cleaning ####
text<-read.csv("GROW_PSF/PSF_data/text_reducer_pumpkinseed_dec.csv") %>%
  select(subject_id, task, 'data.aligned_text', 'data.number_views', 'data.consensus_score','data.consensus_text', "URL.4", "URL.8") %>%
  drop_na('data.number_views') %>%
  naniar::replace_with_na(replace = list(data.consensus_text = c('-', '--', "---", '...', '-------------'))) %>% #function that replaces all of a value with NA 
  drop_na('data.consensus_text') %>%
...
text_bad<- filter(text, data.consensus_score < 2) %>% 
  select(subject_id, column_name, data) %>% 
  filter(subject_id !=82937340 ) %>% 
  filter(subject_id !=82937342)
#checked all of these manually, most correct, but 82937340 and 82937342 rows should not be there 

#join manually reviewed and good datasets 
lengths_clean<-rbind(text_good, text_bad)%>%
  pivot_wider(id_cols = c(subject_id),
              names_from = column_name, 
              values_from = c(data), 
              values_fill = list(data = NA) )
...
#*dropdown ages cleaning #### 
dropdown<-read.csv("GROW_PSF/PSF_data/dropdown_reducer_pumpkinseed_dec.csv", na.strings = c("", "NA")) %>%
  left_join(grow_task_values) %>% 
  select(subject_id, column_name, 'data.value') %>%
  mutate(data_value = gsub("'0 / 0'", "0", data.value), #fix hex codes 
         data_value = gsub("'I / 1'", "1", data_value),
...
# keep >=  2 
age_good<-filter(dropdown, review >= 2 | review2 >= 2 ) %>%
  select(subject_id, column_name, age1)

#this row should not be there 
age_bad<-filter(dropdown, review < 2 &  is.na(review2) ) 

#leave out bad dropdown, so just pivot good 
age_data<-select(age_good, subject_id, column_name, age1) %>%
  pivot_wider(id_cols = c(subject_id),
              names_from = column_name, 
              values_from = c(age1), 
...
all_grow<-left_join(age_data, lengths_clean) %>% 
  ...
  select(-c(row)) %>% 
  mutate(age_group = ifelse(subject_id == 82942106 & length_mean == 9.3, 9, age_group) ) %>%  #after looking at IQR, think this is a typo - age should be 9 (natural progression from front of card)
  mutate(length_mean = ifelse(fish_count == 0, NA, length_mean)) #if fishount is 0 then length should be NA 
...
GROW<-left_join(all_grow, grow_dates) %>%
  left_join(lake_match) %>%
  left_join(grow_units) %>% #units 
  drop_na(url_front) %>% #think these are pages 
   mutate(units= case_when(
    unit == "centimeters" ~ "centimeters",
    unit == "millimeters" ~ "millimeters",
    TRUE ~ 'inches'   )) %>%
...
grow_psf<-read.csv("GROW_PSF/psf_grow_qaqc.csv")

grow_join<-rbind(grow_psf, GROW)

write.csv(grow_join, "GROW_PSF/psf_grow_all_qaqc.csv", row.names = FALSE)
