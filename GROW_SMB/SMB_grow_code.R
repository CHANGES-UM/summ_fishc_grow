...
text<-read.csv("GROW_SMB/SMB_data/text-reducer-SMB-texts.csv") %>%
...
#write.csv(text_bad,  "/Users/katelynking/Desktop/grow_smallmouthbass_review.csv", row.names = FALSE )
...
manual_length<-read.csv("GROW_SMB/SMB_data/grow_smallmouthbass_text_review.csv", na.strings = c("", "NA")) %>%
...
dropdown<-read.csv("GROW_SMB/SMB_data/dropdown-reducer-SMB-dropdowns.csv", na.strings = c("", "NA")) %>%
  ...
  mutate(review1 = gsub("X\\+", "10", review1), #change X+ to 10 
         review2 = gsub("X\\+", "10", review2), 
         review3 = gsub("X\\+", "10", review3), 
         review4 = gsub("X\\+", "10", review4), 
        review1 = gsub("[[:punct:]]", "", review1), #remove punctuation 
...
#write.csv(age_bad, "/Users/katelynking/Desktop/grow_smallmouthbass_ages_review.csv", row.names = FALSE)
age_manual<-read.csv("GROW_SMB/SMB_data/grow_smallmouthbass_ages_review.csv", na.strings = c("", "NA")) %>%
...
all_grow<-left_join(age_data, lengths_clean) %>% 
  ...
  mutate(age_group = ifelse(subject_id == '58642766' & length_min == 18.4, NA, age_group), #correction 
         age_group = ifelse(subject_id == '58753955' & length_min == 19.8, 6, age_group)) #typo 
...
GROW<-left_join(all_grow, grow_dates) %>%
  ...
  filter(!(subject_id == 59962579 & age_group == 11 )) %>% #check units for outliers - this obs is walleye not SMB
  filter(!(subject_id == 59963274)) #59963274 is a copy of 59963257 and should be removed 
...
#final data set 
write.csv(GROW, "GROW_SMB/smb_grow_qaqc.csv", row.names = FALSE)


#### add in December data #### 
#*text (length info) cleaning ####
text<-read.csv("GROW_SMB/SMB_data/text_reducer_smb_dec.csv") %>%
...
#separate into four columns, one for each of the reviews if there are different answers
dropdown<-read.csv("GROW_SMB/smb_data/dropdown_reducer_smb_dec.csv", na.strings = c("", "NA")) %>%
  left_join(grow_task_values) %>% 
  select(subject_id, column_name, 'data.value') %>%
  drop_na('data.value') %>% 
...
#cannot read the card well enough to know what the ages are. leave both obs out
...
#*join age data: dropdown ages and length text  ####
all_grow<-full_join(age_data, text_good) %>% 
  pivot_longer(-subject_id,
               names_to =c("row", ".value"),
               names_sep ="_row") %>% 
  rename(age_group = " age group ", fish_count = " Number of Fish ", length_range = " Length Range ", length_mean = " Mean Length ") %>% 
...
grow_smb<-read.csv("GROW_SMB/smb_grow_qaqc.csv")


grow_join<-rbind(grow_smb, GROW)

write.csv(grow_join, "GROW_SMB/smb_grow_all_qaqc.csv", row.names = FALSE)
